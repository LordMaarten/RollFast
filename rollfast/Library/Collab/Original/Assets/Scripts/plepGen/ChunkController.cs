...
public class ChunkController : MonoBehaviour
{
        ...
        for (int x = 0; x < chunksRendered.x; x++)
        {
            for (int y = 0; y < chunksRendered.y; y++)
            {
                for (int z = 0; z < chunksRendered.z; z++)
                {
                    ...
                    {
                        generateChunk(x, chunkOffsetX, z, chunkOffsetZ, y, chunkOffsetY, chunkX, chunkY, chunkZ);
                    }
                }
            }
        }
    ...
    private void generateChunk(int x, float chunkOffsetX, int z, float chunkOffsetZ, int y, float chunkOffsetY, float chunkX, float chunkY, float chunkZ)
    {
        Stopwatch stopWatch = new Stopwatch();
        stopWatch.Start();
        
        Chunk chunk = new Chunk();
        // chunk.generateChunk(SimplexNoiseGenerator, new Vector3(x + chunkOffsetX, 0, z + chunkOffsetZ), chunkSize, noisePreset);
        var m = pxg.Generator.Mesh.GenerateMesh((int) chunkSize.x, chunkSize, noisePreset.tau, simplexNoiseGenerator);
